...
class MqttClient {
     ...
     * @param {string} options.wifiDataTopic
     * @param {boolean} options.wifiHttpPoolingUrl
    ...
    constructor(options) {
        ...
        this.wifiDataTopic = options.wifiDataTopic ?? "valetudo/robot/WifiConfigurationCapability/signal";
        this.wifiHttpPoolingUrl = options.wifiHttpPoolingUrl ?? false;
        ...
        if (!this.client || (this.client && !this.client.connected && !this.client.reconnecting)) {
            ...
            this.client.on("connect", () => {
                ...
                this.client.subscribe([
                    this.mapDataTopic,
                    this.wifiDataTopic && !this.wifiHttpPooling ? this.wifiDataTopic : null
                ].filter(Boolean), err => {
                    if (!err) {
            ...
            this.client.on("message", async (topic, message) => {
                try {
                    if (topic === this.wifiDataTopic) {
                        const signal = parseInt(message.toString());
                        if (!isNaN(signal) && signal < -1 && signal > -100) {
                            this.mapDrawer.updateWifiSignalLocations(signal);
                        } else {
                            Logger.error("Invalid signal strength received", signal);
                        }
                    } else if (topic === this.mapDataTopic) {
                        ...
                        if (this.wifiHttpPoolingUrl) {
                            await this.mapDrawer.updateWifiSignalByHttpPooling(this.wifiHttpPoolingUrl).catch(Logger.error);
                        }

                        this.mapDrawer.updateRobotPosition(rawMapData);
