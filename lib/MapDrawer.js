...
class MapDrawer {
    ...
    async updateWifiSignalByHttpPooling(url) {
        const response = await axios.get(new URL("/api/v2/robot/capabilities/WifiConfigurationCapability", url).href);
        if (response.status !== 200){
            Logger.error("Unable to get wifi signal from http pooling", response.status);
        }
        try {
            const wifiSignal = response.data.details.signal;
            this.updateWifiSignalLocations(wifiSignal);
        } catch (e){
            Logger.error("Unable to parse wifi signal from http pooling", e);
        }
    ...
    updateWifiSignalLocations(wifiSignal) {
        if (isNaN(wifiSignal) || wifiSignal > -1 || wifiSignal < -100) {
            Logger.error("Invalid signal strength received", wifiSignal);
            return;
        }
